 ...
 * @since 4.0.0
 */
class WC_First_Data_Payeezy_API_Transaction_Response extends WC_First_Data_Payeezy_API_Response implements Framework\SV_WC_Payment_Gateway_API_Authorization_Response, Framework\SV_WC_Payment_Gateway_API_Create_Payment_Token_Response {
	...
	public function transaction_approved() {
		...
		return self::TRANSACTION_STATUS_APPROVED === $this->transaction_status && self::VALIDATION_STATUS_SUCCESS === $this->validation_status;
	}
	...
	protected function get_status_info() {
		...
		if ( $this->transaction_approved() ) {
			...
			if ( $this->has_gateway_error() ) {
				...
				$status->code    = $this->gateway_resp_code;
				$status->message = $this->gateway_message;
	...
	public function has_gateway_error() {
		...
		return '00' !== $this->gateway_resp_code;
	}
	...
	/**
	 * Return the transaction tag, which is the backend gateway (GGe4) identifier
	 * and is required for capturing/refunding
	 *
	 * @since 4.0.0
	 * @return string
	 */
	...
	/**
	 * Return the correlation ID for this transaction
	 *
	 * @since 4.0.0
	 * @return mixed
	 */
	...
	public function get_payment_token() {
		...
		return new Framework\SV_WC_Payment_Gateway_Payment_Token( $this->token->token_data->value, [
			'type'      => 'credit_card',
	...
	public function get_user_message() {
		...
		/**
		 * First Data Gateway Response User Message Filter.
		 *
		 ...
		 * @param \WC_First_Data_Payeezy_API_Transaction_Response $this response class instance
		 */
		return apply_filters( 'wc_first_data_payeezy_response_user_message', $message, $message_id, $this );
	}
	...
	public function get_payment_type() {
		...
		return $this->get_request() instanceof \WC_First_Data_Payeezy_API_Credit_Card_Transaction_Request ? 'credit-card' : 'echeck';
	}
