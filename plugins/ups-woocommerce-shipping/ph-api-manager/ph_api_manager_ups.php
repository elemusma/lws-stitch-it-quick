...
if ( ! class_exists( 'PH_UPS_API_Manager' ) ) {
	class PH_UPS_API_Manager {
		...
		// To support UPS Registration
		public $ph_key 						= '';
		public $ph_prev_api_key_key         = '';
		public $ph_product_order_api_key    = '';

		public $ph_ups_api_history_title  	= '';
		public $ph_ups_api_history_tab_key	= '';

		public $ph_api_history;
		...
		public function __construct( $file, $product_id, $software_version, $plugin_or_theme, $api_url, $software_title = '', $text_domain = '' ) {
			...
			if ( is_admin() ) {
				...
				$this->ph_key            			= 'ph_client_ups';
				$this->ph_prev_api_key_key  		= $this->ph_key . '_prev_api_key';
				$this->ph_product_order_api_key 	= $this->ph_key . '_product_order_api_key';
				$this->ph_ups_api_history_title  	= esc_html__( 'API History', $this->text_domain );
				$this->ph_ups_api_history_tab_key   = $this->data_key . '_api_history';
				...
				if ( !class_exists('PH_UPS_Api_History') ) {

					include_once('ph_ups_api_history.php');
				}

				$this->ph_api_history = new PH_UPS_Api_History();
			}
		...
		public function config_page() {
			...
			if ( $this->ph_api_history->ph_ups_api_history_table_exists() ) {

				$settings_tabs[$this->ph_ups_api_history_tab_key] = esc_html__( $this->ph_ups_api_history_title, $this->text_domain );
			}
            ...
            <div class='wrap'>
				...
				<p><a href="https://www.pluginhive.com/knowledge-base/setting-woocommerce-ups-shipping-plugin/#activate_license" target="_BLANK"><?php _e( "How to Activate the plugin license?", "ups-woocommerce-shipping" ); ?></a></p>

				<p><?php _e( "If you are still facing difficulties, please follow the below steps:", "ups-woocommerce-shipping" ); ?></p>

				<ul class="ph_api_key_activation_steps">
					<li><p><?php _e( "Go to <a href='https://www.pluginhive.com/my-account/api-keys/' target='_BLANK'>PluginHive > My Account > API Keys</a> Section", "ups-woocommerce-shipping" ); ?></p></li>
					<li><p><?php _e( "Delete the previous activation of the license key (If any)", "ups-woocommerce-shipping" ); ?></p></li>
					<li><p><?php _e( "Deactivate and reactivate the UPS plugin", "ups-woocommerce-shipping" ); ?></p></li>
					<li><p><?php _e( "Verify the API key details again and activate the API Key by clicking Save Changes", "ups-woocommerce-shipping" ); ?></p></li>
					<li><p><?php _e( "If you still facing difficulties, kindly contact <a href='https://www.pluginhive.com/support/' target='_BLANK'>PluginHive Support</a> with the WordPress Administrator Access, along with the PluginHive Purchase Order Details. We will activate it on your site promptly.", "ups-woocommerce-shipping" ); ?></p></li>
				</ul>
                ...
                <form action='options.php' method='post'>
                    <div class="main">
						...
						if ( $tab == $this->wc_am_activation_tab_key ) {
							...
							$this->ph_api_history->ph_fetch_ups_api_history_table_data();
						}
						?>
                    </div>
                </form>
		...
		public function validate_options( $input ) {
			...
			if ( $_REQUEST[ 'option_page' ] != $this->wc_am_deactivate_checkbox_key ) {
				if ( $activation_status == 'Deactivated' || $activation_status == '' || $api_key == '' || $checkbox_status == 'on' || $current_api_key != $api_key ) {
					...
					// The $current_api_key is passed as prev key when latest license $api_key is added
					if ( $current_api_key != $api_key ) {

						$ph_prev_api_key = $current_api_key;
					}

					// Backward Compatibility - To avoid sending current activation key in prev key
					if($ph_prev_api_key == $api_key) {

						$prev_activation_details = $this->ph_api_history->ph_get_last_activation_details();

						if( !empty($prev_activation_details) && is_object($prev_activation_details) && $prev_activation_details->product_order_key != $api_key ) {

							$ph_prev_api_key = $prev_activation_details->product_order_key;
						}
					}
					...
					if ( $activate_results[ 'success' ] === true && $activate_results[ 'activated' ] === true ) {
						...
						// Log the Activation Details by default
						Ph_UPS_Woo_Shipping_Common::phAddDebugLog('------------------------------- PH UPS License Activation Details -------------------------------', true);
						Ph_UPS_Woo_Shipping_Common::phAddDebugLog(print_r($args, 1), true);
						Ph_UPS_Woo_Shipping_Common::phAddDebugLog(print_r($activate_results, 1), true);
						...
						if( isset( $activate_results[ 'data' ] ) && isset( $activate_results[ 'data' ][ 'product_order_api_key' ] ) ) {
							...
							update_option( $this->ph_prev_api_key_key, $ph_prev_api_key );
							update_option( $this->ph_product_order_api_key, $activate_results[ 'data' ][ 'product_order_api_key' ] );

							do_action('ph_wc_ups_plugin_license_activated', $activate_results['data']['product_order_api_key']);
							
							$this->ph_api_history->ph_insert_data_to_ups_api_history_table($activate_results);
						}
