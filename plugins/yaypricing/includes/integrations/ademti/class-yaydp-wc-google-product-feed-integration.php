...
class YAYDP_WC_Google_Product_Feed_Integration {
	...
	protected function __construct() {
		if ( class_exists( 'WoocommerceGpfFeedItem' ) ) {
	...
	public function change_feed_item( $feed_item, $product ) {
		...
		$feed_item->sale_price_ex_tax = \wc_get_price_excluding_tax( $product, ['price' => $min_discounted_price] );
		$feed_item->sale_price_inc_tax = \wc_get_price_including_tax( $product, ['price' => $min_discounted_price] );
