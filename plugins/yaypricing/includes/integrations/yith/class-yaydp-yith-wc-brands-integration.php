...
class YAYDP_YITH_WC_Brands_Integration {
	...
	protected function __construct() {
		...
		add_action( 'init', array( $this, 'initialize_yith_brands_integration' ), 100 );
	}

	/**
	 * Initialize callback
	 */
	public function initialize_yith_brands_integration() {
		...
		$yith_brands_taxonomy = \YITH_WCBR::$brands_taxonomy;
		add_filter( "yaydp_admin_custom_filter_{$yith_brands_taxonomy}_result", array( __CLASS__, 'get_filter_result' ), 10, 5 );
		add_filter( "yaydp_get_matching_products_by_{$yith_brands_taxonomy}", array( __CLASS__, 'get_matching_products' ), 10, 4 );
		add_filter( "yaydp_check_condition_by_{$yith_brands_taxonomy}", array( __CLASS__, 'check_condition' ), 10, 3 );
	}
	...
	public function admin_product_filters( $filters ) {
		$yith_brands_filter_array = array(
			array(
			'value'   		    => \YITH_WCBR::$brands_taxonomy,
			'label'            => 'YITH Brands',
			'comparations'     => array(
					array(
						'value' => 'in_list',
		...
		return array_merge( $filters, $yith_brands_filter_array );
	}
