	...
	protected function __construct() {
		...
		if ( class_exists( 'Yay_Currency\Engine\FEPages\WooCommerceCurrency' ) ) {
			add_action ( 'woocommerce_cart_calculate_fees', function(){
				remove_action('woocommerce_cart_calculate_fees', [\Yay_Currency\Engine\FEPages\WooCommerceCurrency::get_instance(), 'recalculate_cart_fees'], 10);
			}, 11 );
		}
	}
		...
		if ( class_exists( '\Yay_Currency\Helpers\Helper' ) ) {
			...
			if ( ! empty( $currencies ) ) {
				$currencies_condition = array(
					...
					'values' => array_map( function( $currency ) use ( $woo_currencies ) {
