...
class YAYDP_Buy_X_Get_Y extends \YAYDP\Abstracts\YAYDP_Product_Pricing_Rule {
	...
	private function get_list_receive_items_for_free_case( $adjustment ) {
		...
		foreach ( $receive_cases['case'] as $a_case ) {
			...
			if ( parent::is_all_together_discount() ) {
				$receive_items = \YAYDP\Core\Discount_Type\YAYDP_Filter_Discount::get_free_receive_items( $cart, $a_case['items'], $receive_quantity, $all_extra_items );
			}
			if ( parent::is_individual_line_item_discount() ) {
				$receive_items = \YAYDP\Core\Discount_Type\YAYDP_Individual_Discount::get_free_receive_items( $cart, $a_case['items'], $receive_quantity, $all_extra_items );
			}
			if ( parent::is_variations_discount() ) {
				$receive_items = \YAYDP\Core\Discount_Type\YAYDP_Variations_Discount::get_free_receive_items( $cart, $a_case['items'], $receive_quantity, $all_extra_items );
			}
	...
	private function get_list_receive_items_for_discount_case( $adjustment ) {
		...
		foreach ( $receive_cases['case'] as $a_case ) {
			...
			if ( parent::is_individual_line_item_discount() ) {
				$receive_items = \YAYDP\Core\Discount_Type\YAYDP_Individual_Discount::get_discount_receive_items( $cart, $a_case['items'], $receive_quantity, $receive_quantity_per_unit, $bought_cases );
			}
			if ( parent::is_variations_discount() ) {
				$receive_items = \YAYDP\Core\Discount_Type\YAYDP_Variations_Discount::get_discount_receive_items( $cart, $a_case['items'], $receive_quantity, $receive_quantity_per_unit, $bought_cases );
			}
			if ( ! empty( $receive_items ) ) {
				$all_receive_items[] = $receive_items;
			}
