...
if (wtp_woocommerce_active_check() ) {
	...
	function wtp_export_data() {
		...
		if (isset($post['wtp_type']) && 'wtp_export_csv' === sanitize_text_field($post['wtp_type']) ) {
			...
			if (isset($post['select_import_type']) && ! empty(sanitize_text_field($post['select_import_type'])) ) {

				if ('product' == sanitize_text_field($post['select_import_type']) ) {
					...
					if (! empty($data) ) {
						...
						$user_input_import_type = sanitize_text_field($post['select_import_type']);
						if (in_array($user_input_import_type, $import_types) ) {
							$temp = $user_input_import_type;
						}
						/**
						 ...
						 * @since 1.0
						**/
						$file_name = str_replace(' ', '_', apply_filters('wtp_change_csv_name', 'wtp_' . sanitize_text_field($post['select_import_type']) . '_tier_pricing.csv'));
				...
				if ('category' == sanitize_text_field($post['select_import_type']) ) {
				...
				if ('user' == sanitize_text_field($post['select_import_type']) ) {
				...
				if ('role' == sanitize_text_field($post['select_import_type']) ) {
	...
	function wtp_general_rule_edit() {
		...
		if (isset($_POST['tier_pricing_nonce'])) {
			check_ajax_referer('tier_pricing_nonce', 'tier_pricing_nonce');
		}
...
*/
	...
	function woo_tier_pricing() {
		include_once WTP_ROOT_PATH . '/product/admin/product_data_tab_content.php';
	}
	...
	function wtp_product_data_tab_content_save( $post_id ) {
		...
		if (isset($_POST['woocommerce_meta_nonce']) ) {
			check_ajax_referer('woocommerce_save_data', 'woocommerce_meta_nonce');
		}
	...
	function wtp_product_variation_data_tab_content( $loop, $variation_data, $variation ) {
		include WTP_ROOT_PATH . '/product/admin/product_data_variation_tab_content.php';
	}
	// save variation
	...
	function wtp_general_role_rule() {
		...
		if (isset($post['rule_id']) && '' != $post['rule_id'] ) {
			...
			if (isset($general_role_rules[ $general_rule_id ]) ) {
				...
				'role'           => sanitize_text_field($post['general_role_user_role']),
				'role_text'      => sanitize_text_field($post['general_role_user_role_text']),
				'min_qty'        => intval($post['general_role_min_quantity']),
				'max_qty'        => intval($post['general_role_max_quantity']),
				'discount_value' => sanitize_text_field($post['general_role_discount_value']),
				'discount_type'  => sanitize_text_field($post['general_role_discount_type']),
				);
	...
	function wtp_general_category_rule() {
		...
		if (isset($post['rule_id']) && '' != $post['rule_id'] ) {
			...
			if (isset($general_category_rules[ $general_rule_id ]) ) {
				...
				'category'       => sanitize_text_field($post['general_category_user_category']),
				'category_text'  => sanitize_text_field($post['general_category_user_category_text']),
				'min_qty'        => intval($post['general_category_min_quantity']),
				'max_qty'        => intval($post['general_category_max_quantity']),
				'discount_value' => sanitize_text_field($post['general_category_discount_value']),
				'discount_type'  => sanitize_text_field($post['general_category_discount_type']),
				);
	...
	function wtp_user_discount_rule() {
		...
		if (isset($post['rule_id']) && '' != $post['rule_id'] ) {
			...
			if (isset($general_user_rules[ $general_rule_id ]) ) {
				...
				'user'           => sanitize_text_field($post['general_user_discount']),
				'user_text'      => sanitize_text_field($post['general_user_discount_text']),
				'min_qty'        => intval($post['general_user_min_quantity']),
				'max_qty'        => intval($post['general_user_max_quantity']),
				'discount_value' => sanitize_text_field($post['general_user_discount_value']),
				'discount_type'  => sanitize_text_field($post['general_user_discount_type']),
				);
	...
	function wtp_setup() {
		...
		load_plugin_textdomain('wtp', false, basename(__DIR__) . '/languages/');
