			...
			foreach ( $general_role_rules as $key => $value ) {
				...
				if ('' === $value['max_qty'] ) {
					...
					$max_qty = $value['max_qty'];
						...
						echo '<tr>';
						echo '<td class="wc-shipping-classes-blank-state col-1" data-value="' . esc_attr($value['role']) . '" colspan="">' . esc_attr($value['role_text']) . '</td>';
						echo '<td class="wc-shipping-classes-blank-state col-2" colspan="">' . esc_attr($value['min_qty']) . '</td>';
						echo '<td class="wc-shipping-classes-blank-state col-3" colspan="">' . esc_attr($max_qty) . '</td>';
						echo '<td class="wc-shipping-classes-blank-state col-4" colspan="">' . esc_attr($value['discount_value']) . '</td>';
						echo '<td class="wc-shipping-classes-blank-state col-5" colspan="">' . esc_attr($value['discount_type']) . '</td>';
			...
			foreach ( $general_user_rules as $key => $value ) {
				...
				if ('' === $value['max_qty'] ) {
					...
					$max_qty = $value['max_qty'];
						...
						echo '<tr>';
						echo '<td class="wc-shipping-classes-blank-state col-1" data-value="' . esc_attr($value['user']) . '" colspan="">' . esc_attr($value['user_text']) . '</td>';
						echo '<td class="wc-shipping-classes-blank-state col-2" colspan="">' . esc_attr($value['min_qty']) . '</td>';
						echo '<td class="wc-shipping-classes-blank-state col-3" colspan="">' . esc_attr($max_qty) . '</td>';
						echo '<td class="wc-shipping-classes-blank-state col-4" colspan="">' . esc_attr($value['discount_value']) . '</td>';
						echo '<td class="wc-shipping-classes-blank-state col-5" colspan="">' . esc_attr($value['discount_type']) . '</td>';
			...
			foreach ( $general_category_rules as $key => $value ) {
				...
				if ('' === $value['max_qty'] ) {
					...
					$max_qty = $value['max_qty'];
						...
						echo '<tr>';
						echo '<td class="wc-shipping-classes-blank-state col-1" data-value="' . esc_attr($value['category']) . '" colspan="">' . esc_attr($value['category_text']) . '</td>';
						echo '<td class="wc-shipping-classes-blank-state col-2" colspan="">' . esc_attr($value['min_qty']) . '</td>';
						echo '<td class="wc-shipping-classes-blank-state col-3" colspan="">' . esc_attr($max_qty) . '</td>';
						echo '<td class="wc-shipping-classes-blank-state col-4" colspan="">' . esc_attr($value['discount_value']) . '</td>';
						echo '<td class="wc-shipping-classes-blank-state col-5" colspan="">' . esc_attr($value['discount_type']) . '</td>' ;
	...
	public function add_admin_field_user_roles_management( $value ) {
		...
		if (! empty($new_user_roles) && count($new_user_roles) > 0 ) {
			foreach ( $new_user_roles as $key => $value ) {
				...
				echo '<tr>';
				echo '<td class="wc-shipping-classes-blank-state" colspan="">' . esc_attr($value['user_role']) . '</td>';
