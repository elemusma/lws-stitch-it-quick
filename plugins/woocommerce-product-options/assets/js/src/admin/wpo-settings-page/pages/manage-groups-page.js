...
import ActionsCell from '../components/tables/actions-cell';
...
const ManageGroupsPage = () => {
	...
	const columns = useMemo(
		() => [
			...
			{
				Header: __( 'Actions', 'woocommerce-product-options' ),
				accessor: '',
				Cell: ( table ) => <ActionsCell table={ table } />,
			},
		],
		[]
	);
		...
		<>
			...
			<ListTable
				...
				compDraggableHeader={
					<WCTableTooltip
						tooltip={ __(
							' Drag and drop to re-order the groups of product options. This will be used for display purposes in the admin, and also on the front end for any products that have more than one group. ',
							'woocommerce-product-options'
						) }
					/>
				}
