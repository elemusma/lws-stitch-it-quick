...
export const getAdvancedSettings = ( option, optionType ) => {
	...
	const settings = Object.entries( option.settings ).filter( entry => {
		const orderedSettings = Object.fromEntries( Object.entries( entry[1] ?? {} ).sort( ( a, b ) => a[0].localeCompare( b[0] ) ).filter( a => a[1] ) );
		const orderedDefaultSettings = Object.fromEntries( Object.entries( defaultAdvancedSettings[ entry[0] ] ?? {} ).sort( ( a, b ) => a[0].localeCompare( b[0] ) ).filter( a => a[1] ) );
		return advancedSettingsKeys.includes( entry[0] ) && JSON.stringify( orderedDefaultSettings ) !== JSON.stringify( orderedSettings );
	} )
...
export const toNumber = ( originalValue ) => {
	if ( originalValue === '' ) {
		return '';
	}
	...
	if ( typeof originalValue === 'string' ) {
		if ( currencyConfig?.thousandSeparator?.length > 0 ) {
			...
			originalValue = originalValue.replace( thousandRegExp, '' );
		}

		if ( currencyConfig?.decimalSeparator?.length > 0 ) {
			const decimalRegExp = new RegExp( `\\${ currencyConfig?.decimalSeparator }`, 'gi' );
			originalValue = originalValue.replace( decimalRegExp, '.' );
		}

		return parseFloat( originalValue );
	} else if ( Object.entries( originalValue )?.[0]?.[0].indexOf( 'wcwp_') === 0 ) {
		// this is a wholesale role pricing object
		const wwpPricing = Object.entries( originalValue );
		const formattedWwpPricing = wwpPricing.map( ( [ key, value ] ) => {
			if ( typeof value === 'string' && value !== '' ) {
				if ( currencyConfig?.thousandSeparator?.length > 0 ) {
					const thousandRegExp = new RegExp( `\\${ currencyConfig?.thousandSeparator }`, 'gi' );
					value = value.replace( thousandRegExp, '' );
				}
	
				if ( currencyConfig?.decimalSeparator?.length > 0 ) {
					const decimalRegExp = new RegExp( `\\${ currencyConfig?.decimalSeparator }`, 'gi' );
					value = value.replace( decimalRegExp, '.' );
				}
			}
