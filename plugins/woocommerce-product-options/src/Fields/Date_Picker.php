...
class Date_Picker extends Abstract_Field {
	...
	public function validate( $value, $option_data ) {
		...
		if ( $restrictions->disable_past_dates && $date < new DateTime( '-1 day' ) ) {
		...
		if ( $restrictions->disable_future_dates && $date > new DateTime( '+1 day' ) ) {
		...
		if ( $restrictions->disable_today && $date->format( 'Y-m-d' ) == ( new DateTime() )->format( 'Y-m-d' ) ) {
		...
		if ( $restrictions->enable_time ) {
			...
			if ( $restrictions->min_time ) {
				$min_time = ( new DateTime( $restrictions->min_time ) )->format( 'H:i' );
			...
			if ( $restrictions->max_time ) {
				$max_time = ( new DateTime( $restrictions->max_time ) )->format( 'H:i' );
	 ...
	 */
	private function get_datetime_object( $date_string ) {
		try {
