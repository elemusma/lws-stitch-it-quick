...
class Plugin_Setup implements Plugin_Activation_Listener, Registerable {
	...
	public function on_activate(): void {
		if ( ! $this->plugin->check_all_requirements() ) {
			set_transient( $this->plugin->get_basename() . '_abort_activation', true );
			return;
		}
	...
	public function after_plugin_activation(): void {
		if ( get_transient( $this->plugin->get_basename() . '_abort_activation' ) && isset( $_GET['activate'] ) ) {
			deactivate_plugins( $this->plugin->get_basename() );
			delete_transient( $this->plugin->get_basename() . '_abort_activation' );
			unset( $_GET['activate'] );
