...
namespace Barn2\Plugin\WC_Product_Options\Dependencies\Lib\Traits;
    ...
    private function check_wp_requirements(string $version, Plugin $plugin) : bool
    {
        ...
        if (\is_admin() && \version_compare($wp_version, $version, '<')) {
            ...
            $admin_notice->add($slug . '_invalid_wp_version', '', \sprintf(
                ...
                __('The %1$s plugin requires WordPress %4$s or greater. Please %2$supdate%3$s your WordPress installation.','woocommerce-product-options' ),
        ...
        if (!\class_exists('WooCommerce')) {
            if (\is_admin()) {
                ...
                $admin_notice->add(
                    ...
                    \sprintf(__('Please %1$sinstall WooCommerce%2$s in order to use %3$s.','woocommerce-product-options' ), Util::format_link_open('https://woocommerce.com/', \true), '</a>', $plugin->get_name()),
                    ['type' => 'error', 'capability' => 'install_plugins', 'screens' => ['plugins']]
                );
                $admin_notice->boot();
            }
        ...
        if (\version_compare($woocommerce->version, $version, '<')) {
            if (\is_admin()) {
                ...
                $admin_notice->add(
                    ...
                    \sprintf(__('The %1$s plugin requires WooCommerce %2$s or greater. Please update your WooCommerce setup first.','woocommerce-product-options' ), $plugin->get_name(), $version),
