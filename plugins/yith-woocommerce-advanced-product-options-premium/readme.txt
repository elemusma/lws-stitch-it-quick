...
Tags: woocommerce product addons, woocommerce custom field, woocommerce custom product addons, product addons woocommerce, WooCommerce product fields
...
4.13.0 - Released on 22 May 2024

* New: support for WooCommerce 8.9
* Tweak: show image even if no image replacement
* Tweak: include a trigger for addon image change
* Tweak: calculate prices via JS, Ajax call only when have a price suffix with shortcodes
* Update: YITH plugin framework
* Fix: uploaded files attached to email
* Fix: update image URL in jquery-ui CSS file and add missing images
* Fix: files already updated when selecting a new variation
* Fix: min/max values for add-on type Upload
* Fix prevent to reduce stock two times for add-ons type product
* Fix: check selected option in select add-on on image replacement reset
* Fix: issue when the number of selected options of an add-on was greater than its exact limit
* Fix: do not show edit product link for bundled items
* Fix: free to premium version error
* Fix: options not added to cart if associated to translation of a product (WPML compatibility)
* Fix: box prices when product price is free and no add-ons are selected
* Fix: add-on value multiplied by number on add-on type Number, included taxes
* Dev: CSS changes for remove icon of add-ons type upload
* Dev: parsing total calculation
* Dev: CSS change for add-ons type color swatch
* Dev: updated 'addon_image_replacement' value by default
...
* Dev: show price on add-on type Select
* Dev: added Subscription compatibility file

4.12.1 - Released on 25 April 2024

* Fix: image not showing on add-on type Selector
* Fix: now showing variation selected in the cart for add-on type Product
* Fix: frontend dependencies in product page and cart page

4.12.0 - Released on 23 April 2024
